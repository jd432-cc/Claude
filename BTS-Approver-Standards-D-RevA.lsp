...
;;;
;;;  Companion module to BTS-Standards-R-v1.1.lsp (BTSStandards.lsp).
;;;  Load release R first. This module uses its register (*BTS-LAYERS*,
;;;  *BTS-REMAP*, *BTS-SPLIT*, *BTS-VARS*) and its helpers rather than
;;;  carrying a second copy of the standard, so the register stays
...
;;;    Help           BTSAHELP
;;;
;;;  Read only : every *CHECK, BTSAUDIT, BTSSTATS, BTSDWGINFO,
;;;              BTSCOORDS, BTSBLOCKREPORT, BTSEXPORT*, BTSCOMPARE,
;;;              BTSPROJECT*, BTSBATCHCHECK.
;;;  Modifies  : BTSLAYERS BTSCLEAN BTSPURGE BTSRECOVER BTSUNITS
;;;              BTSVPLOCK BTSVPFREEZE BTSBLOCKPURGE BTSAUTOFIX
;;;              BTSAUTOREMAP BTSAUTOCLEAN BTSBATCHFIX. Work on a copy.
;;; ==================================================================
...
;;; ---- Release R supplies bts:say, bts:pad, bts:split, bts:uniq,
;;;      bts:lay-ent, bts:lw->str, bts:trans-pct, bts:set-dxf,
;;;      bts:mk-layer, bts:set-lay-xdata, bts:remap-of, bts:review-of,
;;;      bts:split-of, bts:lay-in-std, bts:move-sel, bts:move-blocks,
;;;      bts:count-on, bts:vp-refreeze and bts:audit. Only the four
;;;      printing helpers are cheap enough to stand in for, so that a
;;;      partial load still says something useful instead of failing
;;;      with "no function definition".

(if (null bts:say) (defun bts:say (s) (princ (strcat "\n" s))))
(if (null bts:pad)
...
;;; Guard for anything that reads the register.
(defun bts:need-std ()
  (if *BTS-LAYERS*
    T
    (progn
      (bts:say "BTS register not loaded.")
      (bts:say "  Load BTSStandards.lsp (release R) first, then retry.")
      nil)))
...
;;;  bts:move-blocks and bts:audit live in release R. The wrappers here
;;;  give them the suite names without a second implementation.
;;; ------------------------------------------------------------------
...
(defun bts:stats ( / out ex svc n len census total p)
  ...
  (setq out (list (cons "Drawing" (getvar "DWGNAME"))
                  (cons "Standard" (strcat (cond (*BTS-STD-VERSION*) (t "?"))
                                           " / " *BTS-A-VERSION*))
...
(defun bts:report-writer (fn title lines / f l)
  (if (setq f (bts:open-out fn))
    (progn
      ...
      (write-line (strcat "Register : " (cond (*BTS-STD-VERSION*) (t "not loaded"))) f)
      (write-line "" f)
...
(defun bts:json-sections (iss / lays vars stats nm e p)
  ...
  (list
    ...
    (cons "register" (cond (*BTS-STD-VERSION*) (t "")))
...
(defun bts:write-script (scr files calls save / f me reg n dwg)
  (setq me  (bts:module-path "BTS-Approver-Standards-D-RevA.lsp")
        reg (bts:module-path "BTSStandards.lsp")
        n   0)
  (if (null me) (setq me "BTS-Approver-Standards-D-RevA"))
  (if (null reg) (setq reg "BTSStandards"))
  (if (setq f (bts:open-out scr))
    (progn
      ...
      (foreach dwg files
        ...
        (write-line (strcat "(load \"" reg "\")(load \"" me "\")") f)
...
(defun C:BTSPLOTCHECK ()
  (if (bts:need-std) (bts:show "BTSPLOTCHECK - plot readiness" (bts:check-plot)))
  (princ))
...
(defun C:BTSGRIDCHECK ()
  (if (bts:need-std) (bts:show "BTSGRIDCHECK - grid consistency" (bts:check-grid)))
  (princ))
...
(defun C:BTSSHEETCHECK ()
  (if (bts:need-std) (bts:show "BTSSHEETCHECK - sheet audit" (bts:check-sheets)))
  (princ))
...
(defun C:BTSBLOCKCHECK ()
  (if (bts:need-std) (bts:show "BTSBLOCKCHECK - block compliance" (bts:check-blocks)))
  (princ))
...
;;; ==================================================================
;;;  Help and load banner
;;; ==================================================================

(setq *BTS-A-HELP* '(
  ("BTSAHELP"        "This list. BTSHELP covers the release R commands.")
  ("-- advanced --"  "")
...
;;; Fold the suite into release R's own help list, once per session.
(if (and *BTS-HELP* (null *BTS-A-LOADED*))
  (setq *BTS-HELP* (append *BTS-HELP*
                           (list '("BTSAHELP"
                                   "The Approver Standards suite - 40 further commands.")))))
(setq *BTS-A-LOADED* T)

(bts:say (strcat "BTS " *BTS-A-VERSION* " loaded."))
(if (null *BTS-LAYERS*)
  (bts:say "  WARNING: BTSStandards.lsp is not loaded - load it for the register.")
  (bts:say (strcat "  Register: " (cond (*BTS-STD-VERSION*) (t "?")) ".")))
(bts:say "  BTSAHELP lists the commands.")
(princ)
